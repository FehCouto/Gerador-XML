...
def init_db():
    """Cria as tabelas se não existirem. Tenta até 10x com intervalo de 2s."""
    import time
    for tentativa in range(10):
        try:
            ...
            print(f"Aguardando banco... tentativa {tentativa + 1}/10 ({e})")
            time.sleep(2)
...
def ler_guias():
    ...
    if os.path.exists(caminho_csv):
        with open(caminho_csv, newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                # Remove chaves None (colunas extras no CSV) e converte None → ''
                guias.append({k: (v or '') for k, v in row.items() if k is not None})
    return guias
